...
! 가정 3. mu는 불변량. 즉 u와 mu는 초기에 선언
! 가정 4. sqrt{g}의 부호는 교정하지 않는다.
program main
    use calc_mod
    use fourier1
    use nc_reader
    ...
    real(8) :: m, q, eps, mu, v_perp,deltax   ! 상수
    real(8) :: x0(3)
    real(8) :: u,dudt, g, n_4   ! 갱신하는 스칼라(u=속력, mu=자기)
    real(8) :: b_para_n, b_n(3), bsup(3), bsub(3), b_abs, e_n(3), x(3), dxdt(3), grad_b(3)
    real(8) :: x_1(3), x_2(3), x_3(3), x_4(3), u_1, u_2, u_3, u_4
    real(8) :: n_1(3), n_2(3), n_3(3)  ! 노트 참고(재활용 가능한 변수들)
    real(8) :: r, z, zeta, dx
    integer :: sindex

!!!! 아래는 spline 계수 저장용 변수들
!!!! for spline3 coeffs
...
! Iteration으로 spline 계수 확보
! Saving Spline3 Coeffs

    allocate(b_abs_a(nc%mnmax_nyq, nc%ns-1), b_abs_b(nc%mnmax_nyq, nc%ns-1), &
    b_abs_c(nc%mnmax_nyq, nc%ns-1), b_abs_d(nc%mnmax_nyq, nc%ns-1))
    allocate(g_a(nc%mnmax_nyq, nc%ns-1), g_b(nc%mnmax_nyq, nc%ns-1), &
    g_c(nc%mnmax_nyq, nc%ns-1), g_d(nc%mnmax_nyq, nc%ns-1))

    allocate(bsubs_a(nc%mnmax_nyq, nc%ns-1), bsubs_b(nc%mnmax_nyq, nc%ns-1), &
    bsubs_c(nc%mnmax_nyq, nc%ns-1), bsubs_d(nc%mnmax_nyq, nc%ns-1))
    allocate(bsubu_a(nc%mnmax_nyq, nc%ns-1), bsubu_b(nc%mnmax_nyq, nc%ns-1), &
    bsubu_c(nc%mnmax_nyq, nc%ns-1), bsubu_d(nc%mnmax_nyq, nc%ns-1))
    allocate(bsubv_a(nc%mnmax_nyq, nc%ns-1), bsubv_b(nc%mnmax_nyq, nc%ns-1), &
    bsubv_c(nc%mnmax_nyq, nc%ns-1), bsubv_d(nc%mnmax_nyq, nc%ns-1))

    allocate(bsupu_a(nc%mnmax_nyq, nc%ns-1), bsupu_b(nc%mnmax_nyq, nc%ns-1), &
    bsupu_c(nc%mnmax_nyq, nc%ns-1), bsupu_d(nc%mnmax_nyq, nc%ns-1))
    allocate(bsupv_a(nc%mnmax_nyq, nc%ns-1), bsupv_b(nc%mnmax_nyq, nc%ns-1), &
    bsupv_c(nc%mnmax_nyq, nc%ns-1), bsupv_d(nc%mnmax_nyq, nc%ns-1))


    allocate(rmnc_a(nc%mnmax, nc%ns-1), rmnc_b(nc%mnmax, nc%ns-1), &
    rmnc_c(nc%mnmax, nc%ns-1), rmnc_d(nc%mnmax, nc%ns-1))
    allocate(zmns_a(nc%mnmax, nc%ns-1), zmns_b(nc%mnmax, nc%ns-1), &
    zmns_c(nc%mnmax, nc%ns-1), zmns_d(nc%mnmax, nc%ns-1))
...
! numerical integration
    ...
    do istep=1,steps
        ...
        call crossp(grad_b, bsub, g, n_1)   ! n_1 get
        n_2=-n_1*(mu/q)  ! 2행
        b_n=bsup-n_2*(mu*q)/(b_abs**2)  ! 3행
        b_para_n=b_abs   ! 4행
        e_n=grad_b*(mu/q) ! 5행
        call crossp(e_n, bsub, g, n_3)   ! n_3 get, 6행
